...
thread_local simdjson::builder::string_builder* encode_buffer =
	nullptr;								// Reused across encode() calls
...
template <typename T> void convert_ondemand_element_to_table(lua_State* L, T& element) {
	static_assert(std::is_base_of<ondemand::document, T>::value ||
					  std::is_base_of<ondemand::value, T>::value,
				  "type parameter must be document or value");
...
static void serialize_data(lua_State* L, int current_depth, int max_depth,
...
inline void serialize_append_bool(lua_State* L,
								  SIMDJSON_BUILTIN_IMPLEMENTATION::builder::string_builder& builder,
								  int lindex) {
...
static void serialize_append_number(
	lua_State* L, SIMDJSON_BUILTIN_IMPLEMENTATION::builder::string_builder& builder, int lindex) {
...
static void serialize_append_string(
	lua_State* L, SIMDJSON_BUILTIN_IMPLEMENTATION::builder::string_builder& builder, int lindex) {
	size_t len;
...
static void
serialize_append_array(lua_State* L,
					   SIMDJSON_BUILTIN_IMPLEMENTATION::builder::string_builder& builder,
					   int table_index, int array_size, int current_depth, int max_depth) {
	bool first = true;
...
static void
serialize_append_object(lua_State* L,
						SIMDJSON_BUILTIN_IMPLEMENTATION::builder::string_builder& builder,
...
static int encode(lua_State* L) {
	...
	if (encode_buffer == nullptr || encode_buffer_size != desired_buffer_size) {
		...
		encode_buffer =
			new SIMDJSON_BUILTIN_IMPLEMENTATION::builder::string_builder(desired_buffer_size);
	...
	if (v_err) {
		return luaL_error(L, "failed to get JSON view from buffer: %s",
						  simdjson::error_message(v_err));
	}
...
static int setEncodeBufferSize(lua_State* L) {
	...
	if ((size_t)buffer_size > DEFAULT_MAX_ENCODE_BUFFER_SIZE) {
		return luaL_error(L, "Encode buffer size must not exceed %zu",
						  (size_t)DEFAULT_MAX_ENCODE_BUFFER_SIZE);
	}
...
class ParsedObject {
  ...
  public:
	ParsedObject(const char* json_file)
		: json_string(padded_string::load(json_file)), parser(new ondemand::parser{}) {
		this->doc = this->parser.get()->iterate(json_string);
	}
	ParsedObject(const char* json_str, size_t json_str_len)
		: json_string(json_str, json_str_len), parser(new ondemand::parser{}) {
		this->doc = this->parser.get()->iterate(json_string);
	}
	~ParsedObject() {
	}
...
static int ParsedObject_atPointer(lua_State* L) {
	ondemand::document* document =
		(*reinterpret_cast<ParsedObject**>(luaL_checkudata(L, 1, LUA_MYOBJECT)))->get_doc();
...
static const struct luaL_Reg arraylib_m[] = {{"at", ParsedObject_atPointer},
